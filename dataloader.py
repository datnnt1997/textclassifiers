...
def load_data(train_file, test_file, pretrain=None, max_length=512, save_dir=None):
    ...
    text_field = Field(tokenize=lambda x: x.split(), unk_token='<unk>', pad_token='<pad>', lower=True,
                       init_token=None, eos_token=None, include_lengths=True, batch_first=True)
    label_field = LabelField()
    fields = [("raw", RawField()), ("text", text_field), ("label", label_field)]
    ...
    if save_dir:
        with open(save_dir + "/vocab.json", "w", encoding="utf-8") as fv:
            vocabs = {"word": text_field.vocab.stoi,
        ...
        with open(save_dir + "/fileds.json", "w", encoding="utf-8") as ff:
            field_vocabs = {
                "doc": text_field.vocab.freqs,
                "label": label_field.vocab.freqs
            }
            json.dump(field_vocabs, ff)
            ff.close()
    print("=" * 73)
    return train_dataset, test_dataset, num_vocab, num_classes, pad_idx, vectors
